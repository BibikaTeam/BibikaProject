...
import imageLoadingSrc from "../../../../../assets/imageLoading.gif";
...
const ImageSelector: FC<ImageSelectorProps> = (props) => {
  ...
  const loaderImage: ImageSrcIdModel = {
    imageId: -1,
    imageSrc: imageLoadingSrc,
  };
  ...
  const uploadHandler = async (event: FormEvent<HTMLInputElement>) => {
    getBase64(event.currentTarget.files?.item(0)).then((data: any) => {
      (async () => {
        const filesTmp = files.slice();
        setFiles([...files, loaderImage]);
        await loadImage(data.split(",")[1]).then((id: any) => {
          setFiles(filesTmp);
          setFiles([...files, { imageSrc: data, imageId: id }]);
        });
      })();
    });
  };
    ...
    return (
      <div className="imageSelector-dragger-container" onClick={handleClick}>
        <p className="ant-upload-drag-icon">
          ...
          >
            <path
              d="M14.7725 34.5V20.2275H0.5V14.7352H14.7725V0.5H20.2648V14.7352H34.5V20.2275H20.2648V34.5H14.7725Z"
              fill="#219CE1"
            />
          </svg>
        </p>
  ...
  const handleImageCardClick = (id: number) => {
    props.onUpdate(
      files.filter((file) => file.imageId !== id).map((file) => file.imageId)
    );
